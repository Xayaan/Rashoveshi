...
class Users extends Model implements RoleableInterface, PermissibleInterface, PersistableInterface, UserInterface {
	...
	protected static $designationsModel = 'App\Designation';
	...
	public function designation() {
		return $this->belongsTo(static::$designationsModel);
	}

	public function scopeUnassociated($query) {
		return $query->select(DB::raw('CONCAT(first_name, " ",middle_name, " ",last_name) AS full_name'), 'id')->whereNotExists(function ($query) {
			$query->select(DB::raw(1))->from('authors')->whereRaw('authors.user_id = users.id');
		})->lists('full_name', 'id');
	}
