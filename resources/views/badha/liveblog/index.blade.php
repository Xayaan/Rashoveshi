         ...
         <td> {{ articleStatus($post->status) }}</td>
          @if(Request::is('posts') && Request::get('sort')==null)
            @if($currentUser->hasAccess('post.update'))
                <td>
                 <button onclick="location.href='{{ route('liveblog.bullets', $post->id) }}'" class="edit tiny ui default button"><i class="edit icon"></i>Edit</button>
          ...
          @else
              <td>
                <button onclick="location.href='{{ route('liveblog.bullets', $post->id) }}'" class="edit tiny ui green button"><i class="list icon"></i>Bullets</button>
