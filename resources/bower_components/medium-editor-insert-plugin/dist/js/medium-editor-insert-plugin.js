...
;(function ($, window, document, undefined) {
    ...
    Core.prototype.positionButtons = function (activeAddon) {
        ...
        if ($p.length) {
            ...
            right = $p.position().right - parseInt($buttons.find('.medium-insert-buttons-addons').css('right'), 10) - parseInt($buttons.find('.medium-insert-buttons-addons a:first').css('margin-right'), 10);
            right = right < 0 ? $p.position().right : right;
            top = $p.position().top + parseInt($p.css('margin-top'), 10);

            if (activeAddon) {
                if ($p.position().right !== $first.position().right) {
                    right = $first.position().right;
                }
...
html+='<div class="medium-insert-meta" data-url="'+ data.url + '"></div>';
